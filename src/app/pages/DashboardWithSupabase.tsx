...
import { getTotaisMes, getGastosPorCategoria, getTransacoesMes } from "../../lib/queries";
...
export default function DashboardWithSupabase() {
  ...
  // ID de usuário fake para teste (você pode substituir com o ID real do usuário logado)
  ...
  useEffect(() => {
    const carregarDados = async () => {
      try {
        ...
        // Buscar totais
        const totaisData = await getTotaisMes(userId);
        setTotais(totaisData);

        // Buscar categorias
        const categoriasData = await getGastosPorCategoria(userId);
        setCategorias(categoriasData);

        // Buscar transações
        const transacoesData = await getTransacoesMes(userId);
  ...
  if (loading) {
    return (
      <div className="p-4 lg:p-6 max-w-[1200px] mx-auto">
        <p className="text-center text-[#777777]">Carregando dados...</p>
      </div>
    );
  }

  const recentTransactions = transacoes.length > 0 ? transacoes : mockTransactions.filter((t) => t.type === "expense").slice(0, 6);

  return (
    <div className="p-4 lg:p-6 max-w-[1200px] mx-auto">
      {error && (
        <div className="mb-4 p-3 bg-[#FFF3CD] border border-[#FFE69C] rounded-lg text-[#856404] text-sm">
          {error}
        </div>
      )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-4">
        ...
        <div className="bg-white rounded-lg p-5 border border-[#E0E0E0]" style={{ boxShadow: "0 2px 8px rgba(0,0,0,0.06)" }}>
          ...
          {categorias.length > 0 ? (
            ...
            <p className="text-center text-[#999999]">Nenhum gasto registrado</p>
          )}
        </div>
      </div>
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p style={{ fontSize: 13, fontWeight: 500 }} className="text-black">
                    {tx.descricao || tx.description}
                  </p>
                  <p style={{ fontSize: 11 }} className="text-[#999999]">
                    {tx.categorias?.nome || tx.category} · {formatDate(tx.data || tx.date)}
                  </p>
                </div>
              </div>
              <span style={{ fontSize: 13, fontWeight: 600 }} className="text-[#D32F2F]">
                -{formatCurrency(tx.valor || tx.amount)}
              </span>
            </div>
          ))}
        </div>
